...
### **with_thread.py**
thread이용해서 mid파일 동시 재생 구현함
- cam()실행하는 스레드 
   - 영역에 있는지 검사해서 해당 건반을 array로 전달함
   - 양손일때 원래 array각각 했는데 한 array로 합쳐서 global변수에 저장함    
   -> threads_list 실행하는 스레드에서 가져다 씀
- threads_list 스레드
   - play_mido()를 이용하여 각 영역에 해당하는 음을 재생함


- **개선할것** 
   1. array가 바뀌었을때만 소리가 나도록 했는데 바뀌는걸 확인하는 부분을 하나로 합치기     
   2. 지금은 바뀔때 마다 list에 넣고 실행해서 원래 누르고 있다가 다른 영역에 들어오면 원래 있던 것도 소리가 또 나는데 이부분을 수정해서 바뀌는 숫자만 소리 나도록하기
